...
export class PortfolioService {
  ...
  async getPortfolio(userId: number): Promise<PortfolioResult> {
    try {
      ...
      const [availableCashString, reservedCashString, userPositions] =
        await Promise.all([
      ...
      const currentStockPrices =
        await this.portfolioRepository.getLatestClosePrices(stockIds); // PriceRow[]

      // Create a map of stock ID to current price
      const stockPriceMap = new Map<number, number | null>(
        currentStockPrices.map((p) => [
          p.instrumentId,
      ...
      const userStockPositions: PositiionDetails[] = userPositions.map(
        (position) => {
          ...
          if (sharesOwned !== 0 && totalMoneyInvested !== 0) {
            ...
            profitLossPercentage =
              ((currentPositionValue + totalMoneyInvested) /
                totalMoneyInvestedAbsolute) *
              100;
          }
      ...
      const spendableCashAmount = Math.max(
        availableCashAmount - reservedCashAmount,
        0,
      );
